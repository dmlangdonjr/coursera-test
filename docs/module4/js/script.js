(function (window) {
  ...
  */
  ...
  const map1 = names.map(x => mapNames(x));

  helloReduce = names.reduce((accumulator, currentValue) => {
  ...
  byeReduce = names.reduce((accumulator, currentValue) => {
    var firstLetter = currentValue.charAt(0).toLowerCase();
  ...
  const map2 = helloReduce.hello.map(x => mapNames(x));
  const map3 = byeReduce.bye.map(x => mapNames(x));
})(window);
